...
11. [Logging](#logging)
12. [Companion Script: View-DCEffectiveAuditPolicy.ps1](#companion-script-view-dceffectiveauditpolicyps1)
...
## Companion Script: View-DCEffectiveAuditPolicy.ps1

A companion script, `View-DCEffectiveAuditPolicy.ps1`, is available for a focused view of the current effective audit policy settings.

**Purpose:**
- Displays all advanced audit policy subcategories and their current effective status (Success, Failure, Success and Failure, No Auditing) as reported by `auditpol /get /category:* /r`.
- Attempts to correlate each displayed subcategory with known Event IDs based on mappings used within the `DC-LogMaster.ps1` script.
- Provides the same critical warning about Group Policy precedence, emphasizing that displayed settings are effective local policies which are typically controlled by GPOs.
- This script is **view-only** and does not make any changes to system configuration.

**Usage:**
```powershell
.\View-DCEffectiveAuditPolicy.ps1 [-LogFilePath <path>]
```
This can be useful for quickly assessing the current state and understanding which Event IDs might be generated by the currently active audit subcategories.
